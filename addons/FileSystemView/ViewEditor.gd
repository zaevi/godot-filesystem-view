...
func _ready():
	...
	$HBox/HBox/Add.icon = get_theme_icon("Add", "EditorIcons")
	$HBox/HBox/Delete.icon = get_theme_icon("Remove", "EditorIcons")
...
func update_view_list():
	...
	for view in views:
		...
		if view.icon != "" and has_theme_icon(view.icon, "EditorIcons"):
			popup.set_item_icon(id, get_theme_icon(view.icon, "EditorIcons"))
		id += 1
...
func load_view(idx: int):
	...
	edit_apply_include.set_pressed(view.apply_include)
	edit_include.text			= view.include
	edit_apply_exclude.set_pressed(view.apply_exclude)
	edit_exclude.text			= view.exclude
	edit_hide_dir.set_pressed(view.hide_empty_dirs)
...
func save_current():
	...
	current_view.apply_include		= edit_apply_include.button_pressed
	current_view.include			= edit_include.text
	current_view.apply_exclude		= edit_apply_exclude.button_pressed
	current_view.exclude			= edit_exclude.text
	current_view.hide_empty_dirs	= edit_hide_dir.button_pressed
...
func _on_Icon_text_changed(new_text):
	if has_theme_icon(new_text, "EditorIcons"):
		edit_icon.right_icon = get_theme_icon(new_text, "EditorIcons")
	else:
